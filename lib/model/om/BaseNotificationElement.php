...
abstract class BaseNotificationElement extends BaseObject  implements Persistent {
	...
	const PEER = 'NotificationElementPeer';
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseNotificationElement:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseNotificationElement:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseNotificationElement:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseNotificationElement:save:post') as $callable)
				{
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseNotificationElement:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseNotificationElement::%s', $method));
	  }

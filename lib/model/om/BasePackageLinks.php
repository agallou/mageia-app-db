...
/**
 * Base class that represents a row from the 'package_links' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BasePackageLinks extends BaseObject  implements Persistent {
	 ...
	 * @return     PackageLinks The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = PackageLinksPeer::ID;
		}
	...
	public function setUrl($v)
	{
		...
		if ($this->url !== $v) {
			$this->url = $v;
			$this->modifiedColumns[] = PackageLinksPeer::URL;
		}
	...
	public function setLanguageId($v)
	{
		...
		if ($this->language_id !== $v) {
			$this->language_id = $v;
			$this->modifiedColumns[] = PackageLinksPeer::LANGUAGE_ID;
		}
	...
	public function setPackageId($v)
	{
		...
		if ($this->package_id !== $v) {
			$this->package_id = $v;
			$this->modifiedColumns[] = PackageLinksPeer::PACKAGE_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = PackageLinksPeer::NUM_COLUMNS - PackageLinksPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating PackageLinks object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = PackageLinksPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BasePackageLinks:delete:pre') as $callable)
			{
			...
			if ($ret) {
				PackageLinksPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BasePackageLinks:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BasePackageLinks:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BasePackageLinks:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				PackageLinksPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = PackageLinksPeer::doInsert($this, $con);
					...
					$affectedRows += PackageLinksPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PackageLinksPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PackageLinksPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = PackageLinksPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getUrl(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PackageLinksPeer::ID)) $criteria->add(PackageLinksPeer::ID, $this->id);
		if ($this->isColumnModified(PackageLinksPeer::URL)) $criteria->add(PackageLinksPeer::URL, $this->url);
		if ($this->isColumnModified(PackageLinksPeer::LANGUAGE_ID)) $criteria->add(PackageLinksPeer::LANGUAGE_ID, $this->language_id);
		if ($this->isColumnModified(PackageLinksPeer::PACKAGE_ID)) $criteria->add(PackageLinksPeer::PACKAGE_ID, $this->package_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PackageLinksPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of PackageLinks (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     PackageLinks Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     PackageLinksPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new PackageLinksPeer();
		}
		return self::$peer;
	}
	...
	public function getLanguage(PropelPDO $con = null)
	{
		if ($this->aLanguage === null && ($this->language_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aLanguage->addPackageLinkss($this);
			 */
		}
	...
	public function getPackage(PropelPDO $con = null)
	{
		if ($this->aPackage === null && ($this->package_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aPackage->addPackageLinkss($this);
			 */
		}
	...
	{
	  if (!$callable = sfMixer::getCallable('BasePackageLinks:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BasePackageLinks::%s', $method));
	  }

...
/**
 * Base class that represents a row from the 'rpm_group' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseRpmGroup extends BaseObject  implements Persistent {
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = RpmGroupPeer::ID;
		}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RpmGroupPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseRpmGroup:delete:pre') as $callable)
			{
			...
			if ($ret) {
				RpmGroupPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRpmGroup:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRpmGroup:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRpmGroup:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RpmGroupPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = RpmGroupPeer::doInsert($this, $con);
					...
					$affectedRows += RpmGroupPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RpmGroupPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RpmGroupPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(RpmGroupPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RpmGroup (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RpmGroup Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * related Rpms from storage. If this RpmGroup is new, it will return
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this RpmGroup has previously been saved, it will retrieve
	 * related SubscriptionElements from storage. If this RpmGroup is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collSubscriptionElements === null) {
			if ($this->isNew()) {
			   $this->collSubscriptionElements = array();
				...
				$criteria->add(SubscriptionElementPeer::RPM_GROUP_ID, $this->id);
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseRpmGroup:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseRpmGroup::%s', $method));
	  }

...
/**
 * Base class that represents a row from the 'rpmfile' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseRpmfile extends BaseObject  implements Persistent {
	 ...
	 * @return     Rpmfile The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = RpmfilePeer::ID;
		}
	...
	public function setRpmPkgid($v)
	{
		...
		if ($this->rpm_pkgid !== $v) {
			$this->rpm_pkgid = $v;
			$this->modifiedColumns[] = RpmfilePeer::RPM_PKGID;
		}
	...
	public function setBuildTime($v)
	{
		...
		if ( $this->build_time !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = RpmfilePeer::BUILD_TIME;
			}
	...
	public function setArchId($v)
	{
		...
		if ($this->arch_id !== $v) {
			$this->arch_id = $v;
			$this->modifiedColumns[] = RpmfilePeer::ARCH_ID;
		}
	...
	public function setRpmId($v)
	{
		...
		if ($this->rpm_id !== $v) {
			$this->rpm_id = $v;
			$this->modifiedColumns[] = RpmfilePeer::RPM_ID;
		}
	...
	public function setSize($v)
	{
		...
		if ($this->size !== $v) {
			$this->size = $v;
			$this->modifiedColumns[] = RpmfilePeer::SIZE;
		}
	...
	public function setArch($v)
	{
		...
		if ($this->arch !== $v) {
			$this->arch = $v;
			$this->modifiedColumns[] = RpmfilePeer::ARCH;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->rpm_pkgid = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->build_time = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->arch_id = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->rpm_id = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->size = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->arch = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->resetModified();
			...
			return $startcol + 7; // 7 = RpmfilePeer::NUM_COLUMNS - RpmfilePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Rpmfile object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($con === null) {
			$con = Propel::getConnection(RpmfilePeer::DATABASE_NAME, Propel::CONNECTION_READ);
		}

		// We don't need to alter the object instance pool; we're just modifying this instance
		// already in the pool.

		$stmt = RpmfilePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		if ($con === null) {
			$con = Propel::getConnection(RpmfilePeer::DATABASE_NAME, Propel::CONNECTION_WRITE);
		}
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseRpmfile:delete:pre') as $callable)
			{
			...
			if ($ret) {
				RpmfilePeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRpmfile:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRpmfile:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRpmfile:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RpmfilePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = RpmfilePeer::doInsert($this, $con);
					...
					$affectedRows += RpmfilePeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RpmfilePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RpmfilePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = RpmfilePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getRpmPkgid(),
			$keys[2] => $this->getBuildTime(),
			$keys[3] => $this->getArchId(),
			$keys[4] => $this->getRpmId(),
			$keys[5] => $this->getSize(),
			$keys[6] => $this->getArch(),
		);
		return $result;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setRpmPkgid($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setBuildTime($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setArchId($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setRpmId($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setSize($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setArch($arr[$keys[6]]);
	}
	...
	{
		$criteria = new Criteria(RpmfilePeer::DATABASE_NAME);

		if ($this->isColumnModified(RpmfilePeer::ID)) $criteria->add(RpmfilePeer::ID, $this->id);
		if ($this->isColumnModified(RpmfilePeer::RPM_PKGID)) $criteria->add(RpmfilePeer::RPM_PKGID, $this->rpm_pkgid);
		if ($this->isColumnModified(RpmfilePeer::BUILD_TIME)) $criteria->add(RpmfilePeer::BUILD_TIME, $this->build_time);
		if ($this->isColumnModified(RpmfilePeer::ARCH_ID)) $criteria->add(RpmfilePeer::ARCH_ID, $this->arch_id);
		if ($this->isColumnModified(RpmfilePeer::RPM_ID)) $criteria->add(RpmfilePeer::RPM_ID, $this->rpm_id);
		if ($this->isColumnModified(RpmfilePeer::SIZE)) $criteria->add(RpmfilePeer::SIZE, $this->size);
		if ($this->isColumnModified(RpmfilePeer::ARCH)) $criteria->add(RpmfilePeer::ARCH, $this->arch);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(RpmfilePeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Rpmfile (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Rpmfile Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getArchRelatedByArchId(PropelPDO $con = null)
	{
		if ($this->aArchRelatedByArchId === null && ($this->arch_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aArchRelatedByArchId->addRpmfiles($this);
			 */
		}
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseRpmfile:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseRpmfile::%s', $method));
	  }

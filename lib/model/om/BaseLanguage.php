...
abstract class BaseLanguage extends BaseObject  implements Persistent {
	 ...
	 * related PackageDescriptions from storage. If this Language is new, it will return
		...
		if ($this->collPackageDescriptions === null) {
			if ($this->isNew()) {
			   $this->collPackageDescriptions = array();
				...
				$criteria->add(PackageDescriptionPeer::LANGUAGE_ID, $this->id);
	 ...
	 */
	public function getPackageDescriptionsJoinPackage($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(LanguagePeer::DATABASE_NAME);
		}
		...
		if ($this->collPackageDescriptions === null) {
			if ($this->isNew()) {
				...
				$this->collPackageDescriptions = PackageDescriptionPeer::doSelectJoinPackage($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Language has previously been saved, it will retrieve
	 * related PackageLinkss from storage. If this Language is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collPackageLinkss === null) {
			if ($this->isNew()) {
			   $this->collPackageLinkss = array();
				...
				$criteria->add(PackageLinksPeer::LANGUAGE_ID, $this->id);
	 ...
	 * actually need in Language.
	 */
	public function getPackageLinkssJoinPackage($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(LanguagePeer::DATABASE_NAME);
		}
		...
		if ($this->collPackageLinkss === null) {
			if ($this->isNew()) {
				...
				$this->collPackageLinkss = PackageLinksPeer::doSelectJoinPackage($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last

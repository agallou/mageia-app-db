 ...
 */
abstract class BaseRssFeed extends BaseObject  implements Persistent {
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = RssFeedPeer::NAME;
		}
	...
	public function setHash($v)
	{
		...
		if ($this->hash !== $v) {
			$this->hash = $v;
			$this->modifiedColumns[] = RssFeedPeer::HASH;
		}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RssFeedPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = RssFeedPeer::doInsert($this, $con);
					...
					$affectedRows += RssFeedPeer::doUpdate($this, $con);
				}
	...
	{
		$pos = RssFeedPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = RssFeedPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
			$keys[2] => $this->getHash(),
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(RssFeedPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RssFeed (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RssFeed Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this RssFeed has previously been saved, it will retrieve
	 * related Notifications from storage. If this RssFeed is new, it will return
		...
		if ($this->collNotifications === null) {
			if ($this->isNew()) {
			   $this->collNotifications = array();
				...
				$criteria->add(NotificationPeer::RSS_FEED_ID, $this->id);
	 ...
	 * Otherwise if this RssFeed is new, it will return
	 * an empty collection; or if this RssFeed has previously
	 * been saved, it will retrieve related Notifications from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in RssFeed.
	 */
	public function getNotificationsJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(RssFeedPeer::DATABASE_NAME);
		}
		...
		if ($this->collNotifications === null) {
			if ($this->isNew()) {
				...
				$this->collNotifications = NotificationPeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
